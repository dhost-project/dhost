...
def create_tree_folder(treeview, path, index):
    for root, dirs, files in os.walk(path):
        if index == 1:
            ...
            for d in dirs:
                index += 1
                treeview['child'].append({'name': d, 'id':index,'child':create_tree_folder({}, os.path.join(root, d), index+1)})
            for f in files: 
                index += 1
            ...
            for d in dirs:
                index += 1
                treeview_tree.append({'name': d, 'id':index,'child':create_tree_folder({}, os.path.join(root, d), index+1)})
            if len (dirs) <= 0:
                for f in files: 
                    index += 1
